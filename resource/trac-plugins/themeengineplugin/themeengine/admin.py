...
class CustomThemeAdminModule(Component):
    ...
    def get_admin_panels(self, req):
        if req.perm.has_permission('THEME_ADMIN'):
            yield 'theme', u'テーマ', 'custom', u'カスタマイズ'
            yield 'theme', u'テーマ', 'advanced', u'CSSによるカスタマイズ'
    ...
    def render_admin_panel(self, req, cat, page, path_info):
        ...
        if page == 'advanced' and os.path.exists(os.path.join(self.env.path, 'htdocs', 'theme.css')):
            data['css'] = open(os.path.join(self.env.path, 'htdocs', 'theme.css')).read().decode('utf-8')
        ...
        if req.method == 'POST':
            ...
            if page == 'advanced':
                f.write(req.args.get('css', '').encode('utf-8'))
            else:
