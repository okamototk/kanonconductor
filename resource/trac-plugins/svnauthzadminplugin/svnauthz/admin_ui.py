...
class SvnAuthzAdminPage(Component):
    ...
    def __init__(self):
        self.repository_type = self.env.config.get("trac", "repository_type", None)
    ...
    def get_admin_panels(self, req):
        if req.perm.has_permission('TRAC_ADMIN') and self.repository_type=='svn':
            yield ('subversion', 'Subversion', 'svnauthz', u'Subversionアクセス権')
    ...
    def render_admin_panel(self, req, cat, page, path_info):
        ...
        for repository, path in [(p.get_repo(), p.get_path()) for p in self.authz.get_paths()]:
            ...
            path_disp_url = ("%s:%s" % (repository, path))
...
#                validpath = self._get_valid_path(paths, url2pathname(urlpath))
                validpath = self._get_valid_path(paths, urlpath)
                if validpath:
...
#        editgroup = url2pathname(req.args.get('editgroup'))
        editgroup = req.args.get('editgroup')
...
#        editpath = url2pathname(req.args.get('editpath'))
        editpath = req.args.get('editpath')
        subject = req.args.get('subject')
        acls = req.args.get('addpathmember_acl')
        paths = [(p.get_repo(), p.get_path()) for p in self.authz.get_paths()]
        validpath = self._get_valid_path(paths, editpath)
        self.log.info("(editpath: %s, subject: %s, acls: %s, paths: %s, validpath: %s)" % (editpath, subject, acls, paths, validpath))
        if not validpath:
	    ...
	    self.log.debug("path_members.append ok")
        except Exception, e:
...
#        editgroup = url2pathname(path_info[path_info.index('/')+1:len(path_info)])            
        editgroup = path_info[path_info.index('/')+1:len(path_info)]
        group = self.authz.find_group(editgroup)
        if group != None:
            data['editgroup_name'] = editgroup
#            data['editgroup_url'] = pathname2url(editgroup)
            data['editgroup_url'] = editgroup
            data['editgroup_members'] = [m.__str__() for m in group]
	...
	self.env.log.debug("SvnAdminAuthz: Group %s not found." % editgroup)
        return None, {} 
...
#        editpath = url2pathname(path_info[path_info.index('/')+1:len(path_info)])            
        editpath = path_info[path_info.index('/')+1:len(path_info)]
        paths = [(p.get_repo(), p.get_path()) for p in self.authz.get_paths()]
        validpath = self._get_valid_path(paths, editpath)
        if validpath:
            data['editpath_name'] = self._get_disp_path_name(validpath[0], validpath[1])
#            data['editpath_url'] = pathname2url(editpath)
            data['editpath_url'] = editpath
            ...
            for member in pathmembers:
                ...
                editpath_members.append({'subject' : member.get_member().__str__(),
                                         'read' : read,
                                         'write' : write})
            data['editpath_members'] = editpath_members 
