...
SELECT '1' AS __color__,
       ...
       NULL as  milestone, '残り時間: ' as status,
       CAST(
       SUM(CASE WHEN EstimatedHours.value = '' OR EstimatedHours.value IS NULL THEN 0
         ELSE CAST( EstimatedHours.value AS DECIMAL ) END) -
       SUM(CASE WHEN totalhours.value = '' OR totalhours.value IS NULL THEN 0
...
SELECT '1' AS __color__,
       ...
       t.milestone as  __milestone__, '残り時間: ' as status,
       CAST(
       SUM(CASE WHEN EstimatedHours.value = '' OR EstimatedHours.value IS NULL THEN 0
         ELSE CAST( EstimatedHours.value AS DECIMAL ) END) -
       SUM(CASE WHEN totalhours.value = '' OR totalhours.value IS NULL THEN 0
...
all_reports = [
    {"title":u"集計対象のチケットのレポート",
     "description": u"現在、集計対象のチケットのレポートは時間ベースのレポートです。どのチケットが時間(作業時間の合計)がかかっているか、どの開発者が何のタスクに費やしているのか知るのに有用です。",
     "reports":billing_reports},
    {"title":u"チケット/時間 レポート",
     "description": u"これらのレポートは、予測時間をレビューするのに有用です。これらのレポートは現在は、開始日と終了日を無視します。",
